# Phase 098 - 백엔드 API 및 데이터 계층

## Objective
콘텐츠·번역·검수 데이터를 관리하는 Flask 기반 백엔드와 DB 스키마를 완성한다. 이번 페이즈는 **버전 관리 구현**에 중점을 둔다.

## Tasks
1. 버전 관리를 위한 API 명세를 세부 설계하고 테스트 코드를 작성한다.
...
- 버전 관리 관련 테스트 커버리지 85% 이상

## Notes
- 관련 산출물과 회의록은 `docs/records/phase-098/` 경로에 보관한다.
