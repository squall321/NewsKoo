...
8. Phase 014~020 전략 계획(경쟁 분석, 가치 제안, KPI 거버넌스, 카테고리/수익/리스크/스토리텔링 계획)은 `/phases/014-020`으로 조회할 수 있으며, 각 페이즈의 작업·산출물·성공 지표·타임라인·가드레일을 JSON으로 제공합니다.
9. Phase 021~027 아키텍처/기술 스택 설계 플랜(시스템 다이어그램, 데이터 흐름, 게이트웨이, 이벤트 버스, 캐시, 스토리지, 보안 토폴로지)은 `/phases/021-027` 엔드포인트에서 확인할 수 있으며, 동일한 가드레일과 PoC/ADR 메타데이터를 포함합니다.
