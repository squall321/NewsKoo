            ...
            "segments": [
                {
                    ...
                    "content_needs": (
                        "25분 집중 세션 안에 6~8건을 검수할 수 있도록 출처·번역 로그·모델 버전을 한 화면에서"
                        " 확인하고 카드형/스트림형을 즉시 비교해야 한다."
                    ),
                    "translation_tone": (
                        '원문 구조를 유지하면서 오류·번역체를 표시해 빠르게 수정할 수 있는 "중립·메모형" 톤'
                        "과 가드레일 준수 메시지를 자동 주석으로 노출한다."
                    ),
                    "monetization_hooks": [
                        "Phase 019 큐레이션 타이머/우선순위 큐와 연결해 운영 비용 절감 KPI를 추적",
                ...
                },
                {
                    ...
                    "content_needs": (
                        "카테고리 6종 톤 가이드를 바로 적용하고 원문 맥락과 법무 메모를 함께 확인해 2차 편집을"
                        " 마칠 수 있어야 한다."
                    ),
                    "translation_tone": (
                        '"카테고리별 펀/프로페셔널 믹스"로 한국 독자에게 자연스러운 punchline을 강조하되 원문 CTA/저자 표기를 유지한다.'
                    ),
                    "monetization_hooks": [
                ...
                },
                {
                    ...
                    "translation_tone": (
                        '"구어체·짧은 문장 + 문화적 각색"으로 번역체를 최소화하고 punchline을 한국식으로 재작성하며 민감 소재는 경고 배지로 명시한다.'
                    ),
                    "monetization_hooks": [
                        "카드형 뷰 하단 광고/추천 슬롯 운영",
                        "Phase 090 개인화 알림 CTA 실험(OPS/Product 번들 HYP-008A)로 CPM/구독 전환 추적",
                    ],
                    "onboarding_defaults": {
                ...
                },
                {
                    ...
                    "content_needs": (
                        "해외 트렌드 요약과 실행 인사이트(데이터 포인트, 참고 링크)가 포함된 스트림형 뷰를 선호하며 브랜드 세이프티나 저작권"
                        " 논란 없는 사례를 원한다."
                    ),
                    "translation_tone": (
                ...
                },
                {
                    ...
                    "translation_tone": (
                        '"포멀·감사 대응" 톤으로 모델 버전, 가드레일 준수, 민감 주제 필터링 현황을 데이터 중심으로 보고한다.'
                    ),
                    "monetization_hooks": [
